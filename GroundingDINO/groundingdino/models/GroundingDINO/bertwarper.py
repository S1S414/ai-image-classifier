...
class BertModelWarper(nn.Module):
    def __init__(self, bert_model):
        ...
        # 兼容 transformers >= 4.44（get_head_mask 被移除）
    ...
    def _convert_head_mask_to_5d(head_mask, num_hidden_layers):
        """兼容旧版 transformers 的 head_mask 转换"""
        if isinstance(head_mask, list):
            return torch.stack([torch.NoneTensor if m is None else m for m in head_mask])
    ...
    def forward(
        ...
        extended_attention_mask: torch.Tensor = self.get_extended_attention_mask(
            attention_mask, input_shape, torch.float32
        )
